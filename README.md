...
Version 13.0.0 (for UBA Version 13.0.0)

Code Release Date: March 27, 2020

Documentation Update: March 27, 2020
...
for all versions from UBA 6.2.0 through UBA 13.0.0.
...
-unn The -u flag allows choice of which UBA version to run (and which
     ...
     -u100, -u110, -u120, and -u130.
     bidiref version 13.0 defaults to UBA 13.0.0 rules and data.
...
Parse BidiRefTest.txt and run the UBA 13.0.0 rules on each
...
Parse BidiTest.txt and run the UBA 13.0.0 rules on each
...
BidiCharacterTest.txt, run the (default) UBA 13.0.0 rules on each
...
can do so. The trace flags supported (as of Version 13.0) are:
...
Unicode 13.0, is 130 characters, but the longer input string
...
allowed to default to the current (UBA 13.0.0) version, then the
...
page at https://www.unicode.org/cldr/utility/
