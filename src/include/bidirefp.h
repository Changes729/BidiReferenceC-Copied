 ...
 * 2019-Nov-19 kenw   Update NUMVERSIONS for UBA120.
 * 2020-Mar-26 kenw   Update NUMVERSIONS for UBA130.
 *                    Updated CJK range constants for Unicode 13.0.
 */
...
extern int Trace ( uint32_t traceValue );
...
extern int convertHexToInt ( uint32_t *dest, char *source );
...
extern BIDIPROP br_GetBC ( uint32_t c );
extern BPTPROP br_GetBPT ( uint32_t c );
extern uint32_t br_GetBPB ( uint32_t c );
