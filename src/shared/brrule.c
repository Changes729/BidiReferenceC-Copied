 ...
 * 2019-Nov-19  kenw   Warning removal.
 * 2020-Mar-27  kenw   Bug fix in Rule N0 implementation.
 */
	...
	while ( !spanningDone )
	{
		...
		if ( Trace ( Trace5 ) )
		{
			printf ( "Spanned run id=%d, level=%d, pos=%d, len=%d\n", spanid,
				spanlevel, (int)( bdu - ctxt->theText ), spanlen );
		}
...
static int br_SeekOpeningBracketMatch ( uint32_t closingcp, int pos )
{
int rc;
int depth;
PAIRINGPTR pair;
