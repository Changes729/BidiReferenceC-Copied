...
**      Copyright 2020 Unicode, Inc. All rights reserved.
 ...
 * 2019-Nov-19 kenw   Updated for Unicode 12.0.
 * 2020-Mar-26 kenw   Updated for Unicode 13.0.
 */
...
static int ubaVersion = UBA130;   /* Default to UBA130 */
static int fileFormat = FORMAT_A; /* Format of input data file */

static char *versionNums[NUMVERSIONS] = {"X.X",
                                         "6.2",
                                         "6.3",
                                         "7.0",
                                         "8.0",
...
static uint32_t traceFlags = 0; /* Trace flags for control of debug output */

/*
 * Trace: check a trace bit
 */
int Trace(uint32_t traceValue)
{
 ...
 */
void TraceOn(uint32_t traceValue)
{
 ...
 */
void TraceOff(uint32_t traceValue)
{
  traceFlags &= ~traceValue;
}
...
int convertHexToInt(uint32_t *dest, char *source)
{
  int           slen;
  int           i;
  char *        sp;
...
int br_GetBCFromLabel(char *src)
{
  ...
  switch(src[0]) {
    ...
    case 'S':
      return (BIDI_S);
    case 'P':
      if(strcmp(src, "PDF") == 0) return (BIDI_PDF);
      if(strcmp(src, "PDI") == 0)
        return (BIDI_PDI);
      else
        return (BIDI_Unknown);
