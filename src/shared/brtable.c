 ...
 *                       path chosen.
 * 2019-Nov-19 kenw   Updates for Unicode 12.0.
 * 2020-Mar-26 kenw   Updates for Unicode 13.0.
 */
...
static void br_PrintErr ( int n, char *s )
{
    if ( Trace ( Trace15 ) )
    {
        ...
        {
            printf ( "Error: Invalid hex digit in code point [%s].\n", s );
        }
    }
}
...
static BDPTR getBidiDataPtr ( uint32_t i )
{
    if ( i <= 0x33FF )
    {
        return ( &(planes01[i] ) );
...
char buffer[512];
    ...
    if ( status == UNICODE_DATA )
    {
        ...
        else if ( version == UBA130 )
        {
            fileName = "UnicodeData-13.0.0.txt";
        }
        else if ( version == UBA120 )
        {
...
int rc;
    ...
    if ( Trace ( Trace0 ) )
    {
        printf ( "Note:  Initialized bidiref 13.0.0 library for UBA version %s\n", GetUBAVersionStr() );
    }
